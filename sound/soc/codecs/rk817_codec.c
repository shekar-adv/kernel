...
static int rk817_restart_dac_digital_clk(struct snd_soc_codec *codec)
{
...
static int rk817_restart_adc_digital_clk(struct snd_soc_codec *codec)
{
	snd_soc_update_bits(codec, RK817_CODEC_DTOP_DIGEN_CLKE,
...
static int rk817_codec_power_up(struct snd_soc_codec *codec, int type)
{
	...
	if (type & RK817_CODEC_CAPTURE) {
		...
		rk817_restart_adc_digital_clk(codec);
			    ...
			    struct snd_soc_dai *dai)
{
	...
	DBG("%s : MCLK = %dHz, pre sample rate = %d, cur sample rate = %dHz\n",
	    __func__, rk817->stereo_sysclk, rk817->rate, rate);
	...
	if ((rk817->rate != rate) &&
		...
		if (ret)
			dev_warn(codec->dev, "%s %d clk_set_rate %d failed\n",
...
static int rk817_probe(struct snd_soc_codec *codec)
{
	...
	chip_name = snd_soc_read(codec, RK817_PMIC_CHIP_NAME);
	chip_ver = snd_soc_read(codec, RK817_PMIC_CHIP_VER);
