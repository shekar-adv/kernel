...
{
	struct snd_soc_codec *codec = codec_dai->codec;
	struct es8311_priv *es8311 = snd_soc_codec_get_drvdata(codec);
...
static int es8311_set_tristate(struct snd_soc_dai *codec_dai, int tristate)
{
	struct snd_soc_codec *codec = codec_dai->codec;
...
static int es8311_mute(struct snd_soc_dai *codec_dai, int mute)
{
	struct snd_soc_codec *codec = codec_dai->codec;
	struct es8311_priv *es8311 = snd_soc_codec_get_drvdata(codec);
...
static int es8311_regs_init(struct snd_soc_codec *codec)
{
	/* reset codec */
	snd_soc_write(codec, ES8311_I2C_REGFA, 0x01);
	msleep(20);
	snd_soc_write(codec, ES8311_I2C_REGFA, 0x00);
...
static int es8311_probe(struct snd_soc_codec *codec)
{
	...
	es8311->codec = codec;
	es8311_regs_init(codec);

	/* Configure optional properties: */
	if (es8311->aec_mode)
		snd_soc_update_bits(codec, ES8311_GPIO_REG44,
					      0x70, es8311->aec_mode << 4);
	if (es8311->adc_pga_gain)
		snd_soc_update_bits(codec, ES8311_SYSTEM_REG14,
					      0x0f, es8311->adc_pga_gain);
	if (es8311->adc_volume)
		snd_soc_write(codec, ES8311_ADC_REG17,
					es8311->adc_volume);
	if (es8311->dac_volume)
		snd_soc_write(codec, ES8311_DAC_REG32,
					es8311->dac_volume);

	return 0;
}

static const struct snd_soc_codec_driver soc_codec_dev_es8311 = {
	.probe			= es8311_probe,
			    ...
			    const struct i2c_device_id *id)
{
	...
	return snd_soc_register_codec(&i2c_client->dev,
...
static void es8311_i2c_shutdown(struct i2c_client *client)
{
	...
	snd_soc_write(es8311->codec, ES8311_I2C_REGFA, 0x01);
}
